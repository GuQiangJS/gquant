...
class Metrics():
    ...
    def report(self):
        ...
        d = {
            ...
            '盈利次数占比': self.x_df[self.x_df['profit'] > 0].shape[0]/self.x_df.shape[0] if self.x_df.shape[0] > 0 else 0,
            '盈利(含交易费)次数': self.x_df[self.x_df['profit_comm'] > 0].shape[0],
            '亏损(含交易费)次数': self.x_df[self.x_df['profit_comm'] < 0].shape[0],
            '盈利(含交易费)次数占比': self.x_df[self.x_df['profit_comm'] > 0].shape[0]/self.x_df.shape[0] if self.x_df.shape[0] > 0 else 0,
